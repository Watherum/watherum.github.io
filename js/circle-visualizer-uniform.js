...
AUDIO.VISUALIZER = (function () {
    ...
    Visualizer.prototype.renderLounge = function () {
        ...
        var cy = this.canvas.height / 2;
        var radius = 140;
        var maxBarNum = Math.floor((radius * 1.5 * Math.PI) / (this.barWidth + this.barSpacing)) + 1;
        ...
        for (var i = 0; i < barNum; i++) {
            var amplitude = this.frequencyData[freqJump];
            var alfa = (i * 2 * Math.PI ) / maxBarNum;
            var beta = (3 * 45 - this.barWidth) * Math.PI / 132;
            var x = 0;
